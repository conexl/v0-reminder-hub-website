...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      const unsubscribeCreated = wsClient.on("reminder.created", (message) => {
        if (message.data) {
          setReminders((prev) => [message.data as Reminder, ...prev])
        }
      })
      
      const unsubscribeUpdated = wsClient.on("reminder.updated", (message) => {
        if (message.data) {
          setReminders((prev) =>
            prev.map((r) => (r.id === (message.data as Reminder).id ? (message.data as Reminder) : r))
          )
        }
      })
      
      const unsubscribeCompleted = wsClient.on("reminder.completed", (message) => {
        if (message.data && typeof message.data === "object" && "id" in message.data) {
          setReminders((prev) =>
            prev.map((r) => (r.id === (message.data as { id: string }).id ? { ...r, status: "completed" as const } : r))
          )
        }
      })
  ...
  const handleCompleteReminder = async (id: string) => {
    try {
      const response = await api.completeReminder(id)
      if (response.success) {
        setReminders((prev) =>
          prev.map((r) => (r.id === id ? { ...r, status: "completed" as const } : r))
        )
      } else {
        setError(response.error?.message || "Не удалось завершить напоминание")
      }
    } catch (err) {
      setError("Ошибка при завершении напоминания")
  ...
  const handleDeleteReminder = async (id: string) => {
    try {
      ...
      if (response.success) {
        ...
        setError(response.error?.message || "Не удалось удалить напоминание")
      }
    } catch (err) {
  ...
  const handleCreateReminder = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        setError(response.error?.message || "Не удалось создать напоминание")
      }
    } catch (err) {
